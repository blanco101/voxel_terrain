...
#define FLOAT_TO_FX_10(v) ((int)((v) * 1024.0f))
#define INT_TO_FX_10(v) ((v) << 10)
#define FX_10_TO_FLOAT(v) ((float)(v) / 1024.0f)
#define FX_10_TO_INT(v) ((v) >> 10)
#define FX_10_MUL(a, b) ((int)(((long long)(a) * (long long)(b)) >> 10))
#define FX_10_DIV2(v) ((v) >> 1)
...
// Camera
static int camera_x     = INT_TO_FX_10(0);
static int camera_y     = INT_TO_FX_10(100);
static int camera_z     = INT_TO_FX_10(0);
static int camera_speed = INT_TO_FX_10(1);
...
static void InitializeEffect(int screen_w, int screen_h, float projection) {
  ...
  for (int z = 1; z < RENDER_DEPTH; ++z) {
    perspective_divisions[z] = FLOAT_TO_FX_10(projection / z);
  }
}
...
#define INNER_LOOP()                                                                          \
  u += du;                                                                                    \
  v -= dv;                                                                                    \
  int iu = FX_10_TO_INT(u) & (active_mip_width_mask);                                         \
  int iv = FX_10_TO_INT(v) & (active_mip_height_mask);                                        \
                                                                                              \
  unsigned int texel = active_mip[iu + iv * active_mip_width];                                \
  int terrain_height = INT_TO_FX_10(texel >> 8);                                              \
                                                                                              \
  int height_delta = camera_y - terrain_height;                                               \
  int yp = FX_10_TO_INT(screen_center_y - FX_10_MUL(height_delta, perspective_divisions[z])); \
                                                                                              \
  unsigned int palette_index = texel & 0xFF;                                                  \
                                                                                              \
  yp *= (yp < h); /* Looks weird when yp > h, but hey, removes the if */                      \
  for (int line_y = highest_drawn_col + 1; line_y <= yp; line_y++) {                          \
    *(row++)          = palette_index;                                                        \
    highest_drawn_col = line_y;                                                               \
  }
...
int VoxelTerrain(unsigned char* buffer, int w, int h) {
  ...
  // Screen center
  int screen_center_y = INT_TO_FX_10(h >> 1);
  ...
  for (int xp = 0; xp < w; xp++) {
    ...
    int du = FLOAT_TO_FX_10(Lerp(-1, 1, (float)xp / (float)w));
    int dv = INT_TO_FX_10(1);
      ...
      switch (event.type) {
        case SDL_MOUSEBUTTONDOWN:
          if (SDL_BUTTON_WHEELDOWN == event.button.button) {
            camera_speed -= FLOAT_TO_FX_10(0.2f);
          } else if (SDL_BUTTON_WHEELUP == event.button.button) {
            camera_speed += FLOAT_TO_FX_10(0.2f);
          }
          if (camera_speed < 0.0f) camera_speed = 0;
          if (camera_speed > INT_TO_FX_10(MAX_CAMERA_SPEED)) camera_speed = MAX_CAMERA_SPEED;
          break;
        case SDL_QUIT:
          quit = 1;
          break;
